function numVax = freadVAXD(fid, number, method,skip)
...
switch method   
    case {'float32', 'single'}
      rawUINT32 = fread(fid,number,'uint32=>uint32',skip);
      numVax = uint32le_to_VAXF( rawUINT32 );
      
    case {'float64', 'double'}
      rawUINT32 = fread(fid,2*number,'uint32=>uint32',skip);%read 2 32bit numbers
    ...
    otherwise
      numVax    = fread(fid, number, method,skip);
